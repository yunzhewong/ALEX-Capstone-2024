...
DURATION = 10
POLYNOMIAL_COEFFICIENTS = [-0.1, 0.7]
# POLYNOMIAL_COEFFICIENTS = [0.1, -0.7]
...
def gather_data(command_func: Callable[[aiosv2.SafeMotor, float], None], duration: float, name: str):
    ...
    def func(twinMotor: aiosv2.TwinMotor, runningTime: float):
        ...
        if(math.isclose(cvp.velocity, round(cvp.velocity), abs_tol=0.01) and runningTime > 0.5):
            print(f'current read from motor = {cvp.current}')
            raise MotorStartMoving("Motor stops moving")
