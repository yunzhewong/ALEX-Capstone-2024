...
# network = "172.20.10.255"
# network = "10.12.0.1"
# network = "169.254.137.207"
...
def saveConfig(server_ip):
    data = {"method": "SET", "reqTarget": "/", "property": "save_config"}
    json_str = json.dumps(data)
    print("Send JSON Obj:", json_str)
    s.sendto(str.encode(json_str), (server_ip, PORT_srv))
    try:
...
def eraseConfig(server_ip):
    data = {"method": "SET", "reqTarget": "/", "property": "erase_config"}
    json_str = json.dumps(data)
    print("Send JSON Obj:", json_str)
    s.sendto(str.encode(json_str), (server_ip, PORT_srv))
    try:
...
def reboot(server_ip):
    data = {"method": "SET", "reqTarget": "/", "property": "reboot"}
    json_str = json.dumps(data)
    print("Send JSON Obj:", json_str)
    s.sendto(str.encode(json_str), (server_ip, PORT_srv))
    try:
...
def rebootMotorDrive(server_ip):
    data = {"method": "SET", "reqTarget": "/", "property": "reboot_motor_drive"}
    json_str = json.dumps(data)
    print("Send JSON Obj:", json_str)
    s.sendto(str.encode(json_str), (server_ip, PORT_srv))
    try:
...
def OTAupdate(server_ip):
    data = {"method": "SET", "reqTarget": "/", "property": "OTA_update"}
    json_str = json.dumps(data)
    print("Send JSON Obj:", json_str)
    s.sendto(str.encode(json_str), (server_ip, PORT_srv))
    try:
...
def controlMode(ctrlMode, server_ip, motor_number):
    data = {"method": "SET", "reqTarget": "/m0/controller/config", "control_mode": 3}
    if motor_number == 0:
...
def inputMode(inputMode, server_ip, motor_number):
    data = {"method": "SET", "reqTarget": "/m0/controller/config", "input_mode": 1}
    if motor_number == 0:
        data["reqTarget"] = "/m0/controller/config"
    elif motor_number == 1:
        data["reqTarget"] = "/m1/controller/config"
...
def setLinearCount(set_linear_count, server_ip, motor_number):
    data = {"method": "SET", "reqTarget": "/m0/encoder", "set_linear_count": 0}
    if motor_number == 0:
...
def getMotionCtrlConfig(server_ip, motor_number):
    ...
    try:
        ...
        # if json_obj.get("status") == "OK":
        #     return (
        #         json_obj.get("control_mode"),
        #         json_obj.get("pos_gain"),
        #         json_obj.get("vel_gain"),
        #         json_obj.get("vel_integrator_gain"),
        #         json_obj.get("vel_limit"),
        #         json_obj.get("vel_limit_tolerance"),
        #     )
    except socket.timeout:  # fail after 1 second of no activity
        print("Didn't receive anymore data! [Timeout]")
...
def getMotorConfig(server_ip, motor_number):
    ...
    try:
        ...
        if json_obj.get("status") == "OK":
            return (
                json_obj.get("current_lim"),
                json_obj.get("current_lim_margin"),
                json_obj.get("inverter_temp_limit_lower"),
                json_obj.get("inverter_temp_limit_upper"),
                json_obj.get("requested_current_range"),
                json_obj.get("current_control_bandwidth"),
            )
    except socket.timeout:  # fail after 1 second of no activity
        print("Didn't receive anymore data! [Timeout]")
...
def getTrapTraj(server_ip, motor_number):
    ...
    try:
        ...
        if json_obj.get("status") == "OK":
            return (
                json_obj.get("accel_limit"),
                json_obj.get("decel_limit"),
                json_obj.get("vel_limit"),
            )
    except socket.timeout:  # fail after 1 second of no activity
        print("Didn't receive anymore data! [Timeout]")
...
# return position, velocity, current
def setPosition(
    position, velocity_ff, current_ff, reply_enable, server_ip, motor_number
):
    data = {
        "method": "SET",
...
def setVelocity(velocity, current_ff, reply_enable, server_ip, motor_number):
    ...
    if reply_enable:
        try:
            ...
            json_obj = json.loads(data.decode("utf-8"))
            print(
                "Position = %.2f, Velocity = %.2f, Current = %.4f \n"
                % (
                    json_obj.get("position"),
                    json_obj.get("velocity"),
                    json_obj.get("current"),
...
def setIOState(dict, reply_enable, server_ip):
    data = {"method": "SET", "reqTarget": "/IO_State", "reply_enable": True}
    data["reply_enable"] = reply_enable
